...
# ----------------------------------------
# If pd_test is one of several IP cores in your
...
# ----------------------------------------
# ACDS 26.1 110 linux 2026.06.24.12:33:40
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "pd_test.pd_test"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [pd_test::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [pd_test::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [pd_test::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [pd_test::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [pd_test::normalize_path "$file"]
  set normalized_dest [pd_test::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [pd_test::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [pd_test::get_design_libraries]]
...
alias dev_com {
  ...
  if [ check_precomp_device $PRECOMP_DEVICE_LIB_FILE $FORCE_MODELSIM_AE_SELECTION $ENABLE_QE_LIBRARY_COMPILATION ] {
    eval  qrun -compile  -parallel -outdir ./libraries -vlog.options $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS -end \
...
alias com {
  ...
  set common_design_files [dict values [pd_test::get_common_design_files "$QSYS_SIMDIR"]]
  
  set design_files [list]
  set design_files [concat $design_files [pd_test::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
